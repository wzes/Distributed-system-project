...
object PrePSQB2Plu {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val train = ss.sparkContext.textFile("data/seq_trade_" + dataset + "train_pluno_b.txt")

    val test = ss.sparkContext.textFile("data/seq_trade_" + dataset + "test_pluno_b.txt")
  ...
  def predict(arrays: Array[String], trainLines: Array[String],
    ...
    // output table
    val writer = new BufferedWriter(new FileWriter("data/seq_pre_" + dataset + "rate_plu.csv"))
    writer.write("vip,type,correct rate")
    writer.newLine()
    for (i <- rates.indices) {
      writer.write(i + "," + "pluno," + (rates(i) * 100).formatted("%.2f"))
      writer.newLine()
    }
    writer.close()
  }
