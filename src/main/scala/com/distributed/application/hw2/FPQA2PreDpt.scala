...
object FPQA2PreDpt {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val train = ss.sparkContext.textFile("data/nor_trade_" + dataset + "train_dptno_b.txt")

    val test = ss.sparkContext.textFile("data/nor_trade_" + dataset + "test_dptno_b.txt")
  ...
  def predict(arrays: Array[String], trainLines: Array[String], testLines: Array[String], singleSortedFreqs: Array[String]): Unit = {
    ...
    // output table
    val writer = new BufferedWriter(new FileWriter("data/nor_pre_" + dataset + "rate_dpt.csv"))
